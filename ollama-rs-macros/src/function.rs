...
use syn::{
  spanned::Spanned as _, Error, Expr, ExprLit, FnArg, Ident, ItemFn, Lit, Meta, MetaNameValue, Pat,
  Type,
};
  ...
  input
    ...
    .map(|arg| {
      let pat_type = match arg {
        FnArg::Receiver(_) => return Err(Error::new_spanned(arg, "self argument is not allowed")),
