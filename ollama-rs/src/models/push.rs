...
pub type PushModelStatusStream =
  std::pin::Pin<Box<dyn tokio_stream::Stream<Item = crate::error::Result<PushModelStatus>> + Send>>;
    ...
    let stream = Box::new(res.bytes_stream().map(|res| match res {
      Ok(bytes) => {
        ...
        match res {
          Ok(res) => Ok(res),
          Err(e) => {
            let err = serde_json::from_slice::<crate::error::InternalOllamaError>(&bytes);
            match err {
